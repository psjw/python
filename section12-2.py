...
# WHERE Retrieve4
param4 =(3,5)
c.execute('SELECT * FROM users WHERE id in (?,?)', param4) #%s, %f, %d
print('param4',c.fetchall())


# WHERE Retrieve5
param4 =(3,5)
c.execute("SELECT * FROM users WHERE id in ('%d','%d')"%(3,4)) #%s, %f, %d
print('param5',c.fetchall())


# WHERE Retrieve6
c.execute("SELECT * FROM users WHERE id =:id1 OR id=:id2 ",{"id1":2,"id2":5}) #%s, %f, %d
print('param6',c.fetchall())


# Dump 출력
with conn:
    with open('C:/ljw-study/python/PYTHON_BASIC/resource/dump.sql','w') as f:
